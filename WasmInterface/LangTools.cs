...
namespace WasmInterface
{
    public class LangTools
    {
        ...
        [JSInvokable]
        public static Task GetVerbConj(string verb_word)
        {
            VerbConjugation vdata = new VerbConjugation();

            //var oo = verb.list.Select(v=> new String[2] { v.verb,v.translation } ).Distinct().ToList();
            
            List<VerbConjugation.FrVerb> vlist = vdata.list.Where(v => v.verb == verb_word).ToList();
        ...
        [JSInvokable]
        public static Task GetVerbConj2(string verb_word)
        {
            VerbConjugation vdata = new VerbConjugation();
